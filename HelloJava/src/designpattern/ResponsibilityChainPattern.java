...
public class ResponsibilityChainPattern {
	public static void main(String[] args) {
		Parser parser = new ParserData(new ParserOperation(new ParserFunction(new Parser() {
			
			@Override
			public CalcNode parseAsNode() {
				System.out.println("所有解析器均无法解析，解析失败。");
				return null;
			}
		})));
		System.out.println("===========================");
...
class ParserFunction implements Parser{
	...
	@Override
	public CalcNode parseAsNode() {
		if (new Random(UUID.randomUUID().hashCode()).nextInt(2) == 1) {
